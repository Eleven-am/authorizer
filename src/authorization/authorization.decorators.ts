...
type ContextMapper<T> = (data: void, context: Request & Record<string, any> | Context) => T;

function filterFalsy <T> (model: string, mapper: ContextMapper<T>): ContextMapper<T> {
    return (data, context) => {
        const result = mapper(data, context);
...
function getRequestBody <T> (model: string, mapper: ContextMapper<T>) {
    return (data: void, context: ExecutionContext) => {
        ...
        return filterFalsy(model, mapper)(data, request);
    };
}
